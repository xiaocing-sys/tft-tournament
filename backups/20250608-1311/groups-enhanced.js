    ...
    if (g.players && g.players.length > 0) {
        for (var pi = 0; pi < g.players.length; pi++) {
            ...
            if (gp.screenshot_path) {
                ...
                verifiedBtnHtml = '<button class="verify-btn text-xs ' + verifyCls + ' px-2 py-0.5 rounded" data-gpid="' + gp.id + '" title="点击验证截图">' + verifyText + '</button>'
                    + '<a href="' + gp.screenshot_path + '" target="_blank" class="text-blue-400 text-xs bg-blue-900/40 px-2 py-0.5 rounded" onclick="event.stopPropagation()">截图</a>';
            }
            var promoteBadge = (gp.placement && gp.placement <= 4) ? '<span class="text-yellow-400 text-xs">晋级</span>' : "";
            ...
            playersHtml += '<div class="px-5 py-3 flex items-center justify-between hover:bg-gray-800/30 transition-colors">'
                ...
                + '<span class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ' + circleCls + '">' + rankDisplay + '</span>'
                + '<div>'
